...
<?php
	...
	$q_expense = $conn->query("SELECT * FROM `expense`") or die($conn->error);
	while($f_expense = $q_expense->fetch_array()){
		$q_transaction = $conn->query("SELECT * FROM `transaction` WHERE `transact_detail` = '$f_expense[expense_id]' && `student_id` = '$_REQUEST[student_id]'") or die($conn->error);
